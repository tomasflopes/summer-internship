...
export class TrxParser {
  parse(xml: string): TestRun {
    ...
    if (!content.TestRun || !content.TestRun.TestDefinitions || !content.TestRun.Results || !content.TestRun.Results.UnitTestResult || !content.TestRun.ResultSummary || !content.TestRun.ResultSummary.Counters || !content.TestRun.Times)
      throw new Error('Invalid file structure');

    const testDefinitionsData = content.TestRun.TestDefinitions.UnitTest;
    const testDefinitions = testDefinitionsData.length === undefined ? [testDefinitionsData] : testDefinitionsData;
    const unitTestResultsData = content.TestRun.Results.UnitTestResult;
    const unitTestResults = unitTestResultsData.length === undefined ? [unitTestResultsData] : unitTestResultsData;

    const testResults = unitTestResults.map((testRun: any) => {
      const testData = testDefinitions.find((testDefinition: any) => testDefinition['@id'] === testRun['@testId']);
    ...
    return {
      ...
      times: {
        ...
        duration: this.getDuration(content.TestRun.Times['@start'], content.TestRun.Times['@finish']),
      }
    };
  }
